  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!res.ok) return;
        const data = (await res.json()) as { men: Row[] };
        setRows(data.men ?? []);
      } catch {
        // ignore
      }
  ...
  return (
    <main className="mx-auto min-h-screen w-full px-3 py-8 sm:px-6">
      <div className="mb-4 grid items-start gap-3 lg:grid-cols-[0.85fr_1.5fr_0.85fr]">
        ...
        <section className="rounded-3xl border-2 border-[#1f1f1f] bg-white p-2 shadow-sm">
          <p className="mb-2 rounded-lg bg-[#111] px-3 py-2 text-sm font-extrabold uppercase tracking-[0.08em] text-white">FULL RANKING - MEN</p>
          <div className="mb-3 flex items-center justify-between rounded-2xl border border-[var(--line)] bg-white px-4 py-3">
            <h1 className="font-head text-3xl text-[var(--bg-deep)]">Full Ranking - Men</h1>
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--line)] bg-white">
            ...
            {filtered.map((row) => {
              ...
              return (
                <div key={`mfull-${row.rank}-${row.team}`} className="grid grid-cols-[90px_1.7fr_120px_130px_120px_120px_90px] items-center gap-3 border-b border-[var(--line)] px-4 py-3 text-sm last:border-b-0">
